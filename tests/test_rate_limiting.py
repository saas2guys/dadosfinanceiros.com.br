...
from users.models import (
    Plan, RateLimitCounter, APIUsage, UsageSummary, PaymentFailure,
    RateLimitService, SubscriptionStatus
)
from users.middleware import (
    DatabaseRateLimitMiddleware, RateLimitHeaderMiddleware,
    UserRequestCountMiddleware, set_payment_failure_flags, clear_payment_failure_flags
)
from users import tasks
from .factories import (
    UserFactory, ActiveSubscriberUserFactory, FreePlanFactory,
    BasicPlanFactory, PremiumPlanFactory
)
...
class TestUsageSummary(TestCase):
    ...
    def test_unique_constraint(self):
        ...
        summary2 = UsageSummaryFactory(
            user=summary1.user,
            ip_address=summary1.ip_address,
            date=summary1.date,
            hour=1 if summary1.hour != 1 else 2
        )
...
class TestRateLimitService(TestCase):
    ...
    def test_check_and_increment_new_counter(self):
        ...
        count = RateLimitService.check_and_increment(
            identifier="user_123",
            endpoint="test-endpoint",
            window_type="hour"
        )
    ...
    def test_get_usage_count_from_database(self):
        ...
        count = RateLimitService.get_usage_count(
            identifier="user_123",
            endpoint="test-endpoint",
            window_type="hour"
        )
    ...
    def test_window_start_calculation(self):
        ...
        counter = RateLimitCounter.objects.get(
            identifier="user_1", window_type="minute"
        )
        ...
        counter = RateLimitCounter.objects.get(
            identifier="user_2", window_type="day"
        )
        ...
        counter = RateLimitCounter.objects.get(
            identifier="user_3", window_type="month"
        )
...
class TestEnhancedPlanModel(TestCase):
    ...
    def test_enhanced_plan_fields(self):
        ...
        plan = EnhancedPlanFactory(
            hourly_request_limit=250,
            monthly_request_limit=75000,
            burst_limit=100,
            is_metered=True
        )
...
class TestMaintenanceTasks(TestCase):
    ...
    def test_cleanup_old_rate_limit_counters(self):
        ...
        self.assertFalse(
            RateLimitCounter.objects.filter(id=old_counter.id).exists()
        )
        self.assertTrue(
            RateLimitCounter.objects.filter(id=recent_counter.id).exists()
        )
    ...
    def test_cleanup_old_api_usage(self):
        ...
        self.assertEqual(deleted_count, 1)
        self.assertFalse(
            APIUsage.objects.filter(id=old_usage.id).exists()
        )
        self.assertTrue(
            APIUsage.objects.filter(id=recent_usage.id).exists()
        )
    ...
    def test_aggregate_hourly_usage(self):
        ...
        APIUsageFactory(
            user=user,
            timestamp=last_hour + timedelta(minutes=50),
            date=last_hour.date(),
            response_status=500,
