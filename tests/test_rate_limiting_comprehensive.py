...
from tests.factories import ActiveSubscriberUserFactory, PlanFactory, UserFactory
...
class RateLimitingRequestCombinationTests(TestCase):
    ...
    def setUp(self):
        ...
        self.basic_plan = PlanFactory(
            name='Basic', price_monthly=Decimal('9.99'), hourly_request_limit=100, daily_request_limit=1000, monthly_request_limit=30000
        )

        self.pro_plan = PlanFactory(
            name='Pro', price_monthly=Decimal('29.99'), hourly_request_limit=1000, daily_request_limit=10000, monthly_request_limit=300000
        )

        # Create users with different plans
        self.free_user = UserFactory(current_plan=self.free_plan, subscription_status=SubscriptionStatus.ACTIVE)

        self.basic_user = UserFactory(current_plan=self.basic_plan, subscription_status=SubscriptionStatus.ACTIVE)

        self.pro_user = UserFactory(current_plan=self.pro_plan, subscription_status=SubscriptionStatus.ACTIVE)

        self.inactive_user = UserFactory(current_plan=self.basic_plan, subscription_status=SubscriptionStatus.INACTIVE)
    ...
    def _test_jwt_authentication_rate_limiting(self):
        ...
        for i in range(6):  # Free plan has 5 hourly requests
            response = self.client.get('/api/profile/', HTTP_AUTHORIZATION=f'Bearer {access_token}')
    ...
    def _test_plan_specific_limits(self, user, expected_hourly_limit):
        ...
        for i in range(min(expected_hourly_limit + 1, 10)):  # Cap at 10 for speed
            ...
            if i < min(expected_hourly_limit, 10):
                self.assertEqual(response.status_code, 200, f"Plan {user.current_plan.name} request {i+1} should succeed")

                # Verify rate limit headers
                if 'X-RateLimit-Limit' in response:
                    self.assertEqual(
                        int(response['X-RateLimit-Limit']), expected_hourly_limit, f"Rate limit header should match plan limit"
                    )
    ...
    def _test_day_window_limits(self):
        ...
        daily_counter = RateLimitCounter.objects.filter(identifier=f"user_{self.free_user.id}", window_type='day').first()
    ...
    def _test_month_window_limits(self):
        ...
        monthly_counter = RateLimitCounter.objects.filter(identifier=f"user_{self.free_user.id}", window_type='month').first()
    ...
    def _test_payment_failure_level(self, restriction_level):
        ...
        PaymentFailure.objects.create(
            user=self.basic_user, failed_at=timezone.now(), restriction_level=restriction_level, restrictions_applied=True
        )
        ...
        if restriction_level == 'suspended':
            self.assertEqual(response.status_code, 402, "Suspended users should get payment required")
        elif restriction_level == 'limited':
            # Limited users might have reduced limits or payment restrictions
            self.assertIn(response.status_code, [200, 402, 429], "Limited users should get normal response, payment warning, or rate limit")
        else:  # warning
            # Warning level might return 402 due to payment middleware being active
            self.assertIn(response.status_code, [200, 402], "Warning level should allow requests or show payment warning")
    ...
    def _test_endpoint_rate_limiting(self, endpoint):
        ...
        for i in range(7):
            try:
                response = self.client.get(endpoint, HTTP_X_REQUEST_TOKEN=token)
        ...
        self.assertGreater(successful_requests, 0, f"Should make at least one successful request to {endpoint}")
    ...
    def _test_cache_hit_behavior(self):
        ...
        counter = RateLimitCounter.objects.filter(identifier=f"user_{self.free_user.id}", window_type='hour').first()
    ...
    def _test_cache_miss_behavior(self):
        ...
        counter = RateLimitCounter.objects.filter(identifier=f"user_{self.basic_user.id}", window_type='hour').first()
    ...
    def _test_race_conditions(self):
        ...
        for i, response in enumerate(responses):
            self.assertEqual(response.status_code, 200, f"Request {i+1} should succeed for basic plan")
