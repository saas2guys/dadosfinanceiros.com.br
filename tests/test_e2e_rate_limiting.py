...
from tests.factories import PlanFactory, UserFactory
from users.models import APIUsage, Plan, RateLimitCounter, RateLimitService
...
class EndToEndRateLimitingTest(TestCase):
    ...
    def setUp(self):
        ...
        self.plan = PlanFactory(
            name='Test Plan', price_monthly=Decimal('9.99'), hourly_request_limit=10, daily_request_limit=100, monthly_request_limit=3000
        )
    ...
    def test_different_time_windows(self):
        ...
        for window in windows:
            ...
            counter = RateLimitCounter.objects.filter(identifier=identifier, endpoint=endpoint, window_type=window).first()
    ...
    def test_api_usage_tracking(self):
        ...
        identifier = f"user_{self.user.id}"
        counter = RateLimitCounter.objects.filter(identifier=identifier, endpoint='test').first()
        self.assertIsNotNone(counter)
...
class RateLimitingSystemHealthTest(TestCase):
    ...
    def test_database_counters_persistence(self):
        ...
        counter = RateLimitCounter.objects.get(identifier=identifier, endpoint=endpoint, window_type='hour')
