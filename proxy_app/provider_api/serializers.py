...
from .enums import DeniedHosts, DeniedParameters, EndpointFrom, EndpointTo
...
class BaseResponseSerializer(serializers.BaseSerializer):
    ...
    def _process_data(self, data: dict) -> dict:
        ...
        for key, value in data.items():
            ...
            if isinstance(value, str):
                ...
                result[key] = [
                    self._process_data(item) if isinstance(item, dict) else (self._process_string(item) if isinstance(item, str) else item)
                    for item in value
                ]
            else:
                result[key] = value
    ...
    def _compile_provider_patterns(self) -> list[re.Pattern]:
        ...
        for denied_host in self._denied_hosts:
            ...
            pattern = re.compile(rf'https?://[^/\s\'"<>]*{escaped_host}[^\s\'"<>]*', re.IGNORECASE)
